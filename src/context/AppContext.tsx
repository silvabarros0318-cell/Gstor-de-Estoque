...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const addMovement = async ({ productId, type, quantity, observation }: any) => {
    ...
    const { data: newMov, error } = await supabase.from('movements').insert({
      product_id: productId, type, quantity, observation
    }).select().single();
  ...
  const inviteUser = async (email: string, role: UserRole) => {
    // For MVP we just insert to invitations. Real app would trigger edge function or email.
    const { error } = await supabase.from('invitations').insert({ email, role });
    if (error) return { success: false, error: error.message };
    // Reload invitations (since token is generated by db)
